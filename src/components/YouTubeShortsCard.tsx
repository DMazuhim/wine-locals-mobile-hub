...
interface Props {
  ...
  handleOpenProduct: (slug: string) => void;
  productsLen: number;
  currentIndex: number;
}

const YouTubeShortsCard: React.FC<Props> = ({
  prod,
  index,
  playingIndex,
  setPlayingIndex,
    ...
    >
      ...
      <div className="absolute top-4 left-4 right-4 z-10">
        <div className="flex space-x-1">
          {Array.from({length: productsLen}).map((_, i) => (
            <div 
              key={i}
              className={`h-0.5 flex-1 rounded-full ${
                i === currentIndex ? 'bg-purple-600' : 'bg-gray-300 bg-opacity-30'
              }`}
            />
          ))}
        </div>
